...
<section class="dashboard-section active">
    <div class="section-header">
        ...
        <a href="{{ route('admin.orders') }}" class="btn btn-secondary">Quay lại danh sách</a>
    </div>
    ...
    <div class="dashboard-row" style="gap: 20px;">
        ...
        <div class="col-8">
            ...
            <div class="dashboard-card" style="margin-bottom: 20px;">
                ...
                {{-- Tổng tiền chi tiết --}}
                <div class="order-totals" style="padding-top: 15px; max-width: 400px; float: right; clear: both; margin-top: 20px;">
                    <div class="total-row" style="display: flex; justify-content: space-between;"><span>Tạm tính:</span> <span>{{ number_format($order->tong_tien) }}₫</span></div>
                    <div class="total-row" style="display: flex; justify-content: space-between;"><span>Giảm giá:</span> <span class="text-danger">-{{ number_format($order->giam_gia) }}₫</span></div>
                    <div class="total-row" style="display: flex; justify-content: space-between;"><span>Phí vận chuyển:</span> <span>{{ number_format($order->phi_van_chuyen) }}₫</span></div>
                    <div class="total-row grand-total" style="display: flex; justify-content: space-between; border-top: 2px solid var(--border-color); padding-top: 10px; margin-top: 10px;">
                        <span style="font-weight: bold;">TỔNG THANH TOÁN:</span> <span class="price-final" style="color: var(--primary-color); font-size: 1.2em; font-weight: bold;">{{ number_format($order->thanh_tien) }}₫</span>
                    </div>
                </div>
            </div>
        ...
        <div class="col-4">
            ...
            <div class="dashboard-card" style="margin-bottom: 20px;">
                ...
                <form action="{{ route('admin.orders.update', $order->id) }}" method="POST">
                    ...
                    {{-- FIX LỖI: Thêm các hidden input để thỏa mãn validation của Controller --}}
            ...
            {{-- Nút Xóa (Soft Delete) --}}
            <form action="{{ route('admin.orders.destroy', $order->id) }}" method="POST" onsubmit="return confirm('Bạn có chắc chắn muốn XÓA VĨNH VIỄN (soft delete) đơn hàng này? Việc này sẽ hoàn lại tồn kho nếu đơn hàng chưa hủy.');" style="margin-top: 20px;">
                @csrf @method('DELETE')
